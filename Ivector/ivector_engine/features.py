...
def framing(a, window, shift=1):
    ...
    return np.lib.stride_tricks.as_strided(a, shape=np.array(shape).astype("int64"), strides=np.array(strides).astype("int64"))
...
def mfcc_htk(x, window, noverlap, fbank_mx, nfft=None,
    ...
    x = framing(x.astype("int64"), window.size, window.size-noverlap).copy()

    if ZMEANSOURCE:
        
        x -= np.array(x.mean(axis=1)[:,np.newaxis]).astype("int64")
